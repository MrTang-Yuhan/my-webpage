...
- 正文字段应只提供 Markdown 源码编辑，不应出现 Rich Text/富文本切换入口。
- Markdown 编辑不应改写 `_`、`-`、列表 marker 或强调 marker。
...
- 使用文章编辑页“上传图片”按钮可打开 Decap 原生媒体库，上传后图片落到当前文章目录的 `img/` 子目录，正文不应被自动修改。
- 使用文章编辑页“上传附件”按钮可打开 Decap 文件上传入口，上传后附件落到当前文章目录的 `attach/` 子目录，正文不应被自动修改，保存后 front matter 不应保留 `attachment_upload`。
