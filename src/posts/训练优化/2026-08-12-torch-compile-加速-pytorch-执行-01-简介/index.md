...
**所有深度学习训练/推理的代码，在正式部署时最好都使用 torch.compile**。
...
**加速来源**：主要来自减少 Python 开销和 GPU 读写次数。实际加速效果因模型架构和 batch size 而异——如果模型架构简单且数据量很大，瓶颈在 GPU 计算，加速可能不那么显著。
