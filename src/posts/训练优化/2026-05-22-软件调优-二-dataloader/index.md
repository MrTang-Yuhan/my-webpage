...
本质上，workers 提升性能的原因是**减少 GPU 等待数据的时间**。[^1]

[^1]: `num_workers > 0` 在极少情况下可能出现 bug，如果遇到则设置 `num_workers = 0`
...
[^2]: pinned memory 禁止换页，即告诉 OS 这块物理页必须留在 RAM 里，不准换到磁盘。所以应该避免分配太多的 pinned memory。
