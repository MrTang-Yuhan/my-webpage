...
|---|---|---|---|---|
| Embedding | $x = \mathrm{Embed}(tokens)$ | embedding 输出 $x$ | $B \times T \times D$ | 中等 |
...
| Attention scores | \(S=QK^\top/\sqrt d\) | 通常不一定保存 \(S\)，但可能保存 mask 后的 scores | \(B \times h \times T \times T\) | 很大 |
...
| LayerNorm 2 | \(v=\mathrm{LN}_2(x')\) | 输入 \(x'\) 或 \(\hat{x}'\)，以及 `mean/rstd` | \(B \times T \times D\) | 中等 |
...
| LM Head / Classifier | \(\ell=xW_{\text{vocab}}\) | 输入 \(x\)，有时保存 logits | logits: \(B \times T \times Vocab\) | 可能极大 |
