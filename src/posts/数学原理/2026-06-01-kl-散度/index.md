...
当我们了解 $X$ 的分布概率 $P$ 时，在知道真相 $X=x_i$ 时，所获得的信息量为：
...
时，由于认为 $X$ 取值为 $x_i$ 的概率为 $q_i$，则在知道 $X=x_i$ 时获得的信息量为：
...
没有掌握真实分布时获得的信息量，肯定比掌握真实分布时获得的信息量更大。

也就是说，当认知分布 $Q(x)$ 与真实分布 $P(x)$ 偏差越大时，交叉熵就比信息熵更大。

因此，用两者的差来定义 $Q(x)$ 与 $P(x)$ 的差异，称为 KL 散度：
...
KL 散度直接评估了交叉熵与信息熵的差异，从而间接地评估了 $P(x)$ 与 $Q(x)$ 的差异。
