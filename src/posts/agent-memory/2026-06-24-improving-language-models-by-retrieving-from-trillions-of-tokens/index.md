...
> 严格来说，它来自**带条件版本的贝叶斯定理**：
> $$P(Y \mid X, D, \theta) = \frac{P(D \mid Y, X, \theta) \cdot P(Y \mid X, \theta)}{P(D \mid X, \theta)}$$
> 这里的 $P(\cdot|X,\theta)$ 是共同条件。
>
> 这里做了一个**核心假设**：
> **检索到的文档 $D$ 只依赖输入 $X$，不依赖输出 $Y$。**
> 
> 即：$P(D \mid Y, X, \theta) = P(D \mid X)$
