...
- Rich Text/所见即所得模式已禁用，避免 `_`、`-`、列表 marker 或强调 marker 被自动规范化。
...
- 新建/编辑文章时，可用文章编辑页的“上传图片”按钮打开 Decap 原生媒体库；图片会写入该文章目录下的 `img` 文件夹，正文不会被自动修改。
- 文章编辑页的“上传附件”按钮会调用 Decap 文件上传控件；附件会写入该文章目录下的 `attach` 文件夹，正文不会被自动修改。
